...
namespace GitUI
{
    public partial class FormGerritDownload : FormGerritBase
    {
        ...
        private bool DownloadChange(IWin32Window owner)
        {
            ...
            string fetchCmd = GitCommands.Settings.Module.FetchManualCmd(
                _NO_TRANSLATE_Remotes.Text,
                refspec
            );

            if (!RunCommand(owner, fetchCmd, change))
                return false;

            string checkoutCmd = GitCommandHelpers.BranchCmd(branchName, "FETCH_HEAD", true);

            bool success = RunCommand(owner, checkoutCmd, change, (ref bool isError, FormProcess processForm) =>
            {
                if (isError)
                {
                    string output = processForm.OutputString.ToString();

                    if (output.Contains("already exists"))
                    {
                        // Recycle the current review branch.

                        if (!RunCommand(owner, "checkout " + branchName, change))
                            return false;

                        string resetCmd = GitCommandHelpers.ResetHardCmd("FETCH_HEAD");

                        if (!RunCommand(owner, resetCmd, change))
                            return false;
        ...
        private bool RunCommand(IWin32Window owner, string command, string change)
        {
            return RunCommand(owner, command, change, null);
        }

        private bool RunCommand(IWin32Window owner, string command, string change, HandleOnExit handleOnExit)
        {
            var form = new FormRemoteProcess(command)
            {
                Remote = _NO_TRANSLATE_Remotes.Text,
                Text = string.Format(_downloadCaption.Text, change),
                HandleOnExitCallback = handleOnExit
            };

            form.ShowDialog(owner);

            return !form.ErrorOccurred();
        }
